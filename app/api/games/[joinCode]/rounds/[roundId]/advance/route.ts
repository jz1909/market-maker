...
import { createGameEvent, GameStartedData, GameEndedData } from
"@/lib/realtime/events";
...
export async function POST(req:Request, {params} : {params: Promise<{joinCode:string, roundId:number}>}) {
    ...
    if( game.makerUserId!== dbUser.id){
        ...
        return NextResponse.json({error: "Only maker can advance the game"}, {status: 403})
    ...
    if (result.gameEnded) {
        ...
        if (winnerId){
            await db.update(games).set({winnerUserId: winnerId}).where(eq(games.id, game.id))
        }
