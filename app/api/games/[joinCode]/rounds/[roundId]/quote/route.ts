...
export async function POST(req:Request, {params}:{params:Promise<{joinCode:string; roundId: string}>}){
    ...
    if(typeof bid !== "number" || typeof ask !== "number"){
        return NextResponse.json({error:"Invalid bid ask values"}, {status: 400})
    }
    ...
    if(game.makerUserId !== dbUser.id){
        return NextResponse.json({error:"Only maker can submit a quote"}, {status:403})
    }
    ...
    if(!isValidQuote(bid,ask)){
        return NextResponse.json({error: "Invalid quote, bid must be less than ask"}, {status: 400})
    }
    ...
    return NextResponse.json({success:true, turnIndex: round.currentTurnIndex})
