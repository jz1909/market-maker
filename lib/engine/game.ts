...
export async function startRound(roundId:string):Promise <void>{
    ...
    await db.update(rounds).set({roundStatus:"LIVE",startedAt:new Date()})
    .where(eq(rounds.id, roundId))

}

export async function submitQuote(roundId:string, turnIndex:number, 
    makerUserId:string, bid:number, ask:number):Promise<void>{
...
export async function executeTrade(roundId:string, turnIndex:number, gameId:string,
    takerUserId:string, side: "BUY" | "SELL" | null, price:number, 
...
export async function advanceTurn(roundId:string):Promise<{nextTurnIndex:number|null;
    roundEnded:boolean;}>{
        ...
        if (nextTurn >= DEFAULT_GAME_CONFIG.turnsPerRound){
            await db.update(rounds).set({roundStatus:"ENDED", endedAt: new Date()})
            .where(eq(rounds.id, roundId))
        ...
        await db.update(rounds).set({currentTurnIndex:nextTurn}).where(eq(rounds.id, roundId))
...
export async function settleRound(roundId:string):Promise<{
    ...
    await db.update(rounds).set({roundStatus:"SETTLED"}).where(eq(rounds.id, roundId))

    return {makerPnL, takerPnL, correctAnswer}
}

export async function advanceGame(gameId:string):Promise<{nextRoundId:string | null;
    gameEnded:boolean;
}>{
...
// use length to represent new last item in array
    ...
    if(currentRoundIndex>=DEFAULT_GAME_CONFIG.totalRounds){
        await db.update(games).set({gameStatus:"FINISHED", finishedAt: new Date()})
        .where(eq(games.id, gameId))
    ...
    .values({gameId, roundIndex:currentRoundIndex, 
    questionId:randomQuestion.id, roundStatus:"PENDING"}).returning();
